...
param_grid = {
    "filters": [32], # "filters": [16, 32],          
    "dropout": [0.2],             
    "l2_reg": [1e-4],             
    "learning_rate": [0.001],# "learning_rate": [0.001, 0.0005],  
    "window_size": [20],          
...
def create_robust_convlstm(time_steps, H, W, filters, activation, dropout, l2_reg, learning_rate):
    model = Sequential([
        # Bloque 1
        ConvLSTM2D(filters=filters, kernel_size=(3,3), padding='same', return_sequences=True,
        ...
        # Bloque 3
        ConvLSTM2D(filters=filters//2, kernel_size=(3,3), padding='same', return_sequences=False,
                   activation=activation, kernel_initializer='glorot_uniform',
                   kernel_regularizer=l2(l2_reg)),
        BatchNormalization(),
        Dropout(dropout),
...
def scheduled_sampling_training(model, X_train, y_train, X_val, y_val, 
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            wait = 0
            model.save_weights("best_weights_ss.h5")  # guardar mejores pesos
        else:
            wait += 1
            if wait >= patience:
                print(f"Early stopping activado en epoch {epoch+1}")
                model.load_weights("best_weights_ss.h5")
                break
    
    return history

# Cargar datos (una sola vez, fuera del bucle)
try:
...
for combo in combinations:
    ...
    print(f"\nEjecutando con parámetros: filters={filters}, dropout={dropout}, l2_reg={l2_reg}, "
          f"learning_rate={learning_rate}, window_size={window_size}, activation={activation}")
    
    # Crear subcarpeta para esta combinación (usando filters, lr, activation en el nombre)
    sub_dir_name = f"resultados_{filters}_{learning_rate}_{activation}"
    sub_dir = os.path.join(model_dir, sub_dir_name)
    os.makedirs(sub_dir, exist_ok=True)
    
    # Crear ventanas deslizantes con el window_size actual
    ...
    model = create_robust_convlstm(time_steps, H, W, filters, activation, dropout, l2_reg, learning_rate)
    model.summary()
    ...
    # Guardar el modelo y scalers en sub_dir
    model.save(os.path.join(sub_dir, "model.keras"))
    joblib.dump(scaler_X, os.path.join(sub_dir, "scaler_X.pkl"))
    joblib.dump(scaler_y, os.path.join(sub_dir, "scaler_y.pkl"))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "mse.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "mse_normalized.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "ssim.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "ssim_normalized.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "predictions_vs_true_zoom100.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "predictions_vs_true.png"))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(sub_dir, "train_val_loss.png"))
    plt.close()
    ...
    output_video = os.path.join(sub_dir, "output_video.mp4")
    ...
    # Guardar métricas
    with open(os.path.join(sub_dir, "metricas.txt"), "w") as f:

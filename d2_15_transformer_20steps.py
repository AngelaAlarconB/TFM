...
def create_spatiotemporal_transformer_final(n_frames=WINDOW_SIZE, H=116, W=116, C=2,
                                            num_heads=4, key_dim=128, num_blocks=4,
                                            dropout=0.1, learning_rate=1e-3, l2_reg=1e-4):
    """
    Spatiotemporal Transformer adaptado a multicanal (C capas).
    Entrada: (B, n_frames, H, W, C)
    Salida:  (B, H, W, C)
    """
    inp = Input(shape=(n_frames, H, W, C), name="input")

    # Aplanamos espacio + canales: (B, n_frames, H*W*C)
    x = Reshape((n_frames, H * W * C), name="flatten_spatial")(inp)
    ...
    last = x[:, -1, :]                                      # (B, key_dim)
    out_vec = Dense(H * W * C, activation='linear', name="to_pixels")(last)
    out = Reshape((H, W, C), name="out_2d")(out_vec)
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + len(mse_per_t)), mse_per_t, label='MSE Transformer')
plt.xlabel('Tiempo t')
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
         ...
         ssim_scores, label='SSIM Transformer')
plt.xlabel('Tiempo t')
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_pred_denorm[:, pixel_idx_x, pixel_idx_y, 0], label='Predicciones Transformer (Capa 1)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_test_denorm[:, pixel_idx_x, pixel_idx_y, 1], label='Valores Reales (Capa 2)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_pred_denorm[:, pixel_idx_x, pixel_idx_y, 1], label='Predicciones Transformer (Capa 2)', alpha=0.7)
plt.xlabel('Tiempo t')
plt.ylabel('Valor')
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_pred_denorm[:100, pixel_idx_x, pixel_idx_y, 0], label='Predicciones Transformer (Capa 1)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_test_denorm[:100, pixel_idx_x, pixel_idx_y, 1], label='Valores Reales (Capa 2)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_pred_denorm[:100, pixel_idx_x, pixel_idx_y, 1], label='Predicciones Transformer (Capa 2)', alpha=0.7)
plt.xlabel('Tiempo t')
plt.ylabel('Valor')
...
plt.title('Pérdida durante el Entrenamiento de Transformer')
plt.grid(True)
plt.legend()
plt.tight_layout()
...
with open(os.path.join(model_dir, "metricas.txt"), "w") as f:
    f.write("Resultados Globales para Transformer:\n")
...
# Imprimir métricas
print("\nResultados Globales para Transformer:")

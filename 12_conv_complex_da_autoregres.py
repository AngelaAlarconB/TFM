...
def create_robust_convlstm(H, W):
    model = Sequential([
        Conv2D(filters=16, kernel_size=(3, 3), padding='same', activation='relu', kernel_initializer=initializer, kernel_regularizer=l2(1e-4),
               input_shape=(H, W, 1), data_format='channels_last'),
        BatchNormalization(),
        Dropout(0.2),
        Conv2D(filters=16, kernel_size=(3, 3), padding='same', activation='relu', data_format='channels_last', kernel_initializer=initializer, kernel_regularizer=l2(1e-4)),
        BatchNormalization(),
        Dropout(0.2),
        Conv2D(filters=1, kernel_size=(3, 3), padding='same', activation='linear', data_format='channels_last', kernel_initializer=initializer, kernel_regularizer=l2(1e-4))
    ])
...
model = create_robust_convlstm(H, W)
model.summary()
...
history = model.fit(X_train_scaled.squeeze(axis=1), y_train_scaled, epochs=EPOCHS, batch_size=BATCH_SIZE, validation_split=0.2, callbacks=[early_stop], shuffle=False, verbose=1)

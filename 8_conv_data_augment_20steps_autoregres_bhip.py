...
from tensorflow.keras.layers import Conv2D, ConvLSTM2D, LeakyReLU
...
def build_model(hp):
    n_filters = hp.Choice("n_filters", [8, 16, 32, 64])
    
    # Usamos string para evitar error
    kernel_size_str = hp.Choice("kernel_size", ["3", "5"])
    kernel_size = (int(kernel_size_str), int(kernel_size_str))
    
    learning_rate = hp.Choice("learning_rate", [1e-3, 5e-4, 1e-4])
    activation = hp.Choice("activation", ["relu", "tanh", "elu", "leaky_relu"])
    ...
    model.add(ConvLSTM2D(
        ...
        padding="same",
        activation=activation if activation != "leaky_relu" else None,
        return_sequences=True,
        kernel_initializer=initializer,
        input_shape=(WINDOW_SIZE, X.shape[2], X.shape[3], 1)
    ))
    model.add(ConvLSTM2D(
        filters=n_filters,
        kernel_size=kernel_size,
...
tuner.search(X_train_scaled, y_train_scaled, epochs=20, batch_size=BATCH_SIZE, validation_split=0.2, callbacks=[stop_early])

best_hp = tuner.get_best_hyperparameters(num_trials=1)[0]
model = tuner.get_best_models(num_models=1)[0]

print("\nMejores hiperparámetros encontrados:")
print(f" Filtros: {best_hp.get('n_filters')}")
print(f" Kernel: {best_hp.get('kernel_size')}")
print(f" Learning rate: {best_hp.get('learning_rate')}")
print(f" Activación: {best_hp.get('activation')}")

...
# --- Data Augmentation ---
def augment_data(X, y, noise_sigma=0.01, dropout_p=0.05, n_dyn_channels=1):
    """
    Aplica data augmentation a los canales dinámicos, dejando intactos los metadatos.
    
    Args:
        X: (n_samples, n_frames, H, W, C_in)
        y: (n_samples, H, W, C_out)
        n_dyn_channels: nº de canales dinámicos (ej: 1 = PA, el resto son metadatos).
    """
    X_aug, y_aug = [X[i] for i in range(X.shape[0])], [y[i] for i in range(y.shape[0])]

    for i in range(X.shape[0]):
        X_dyn = X[i][..., :n_dyn_channels]   # canales dinámicos (ej: PA)
        X_meta = X[i][..., n_dyn_channels:]  # metadatos (se dejan igual)

        # Ruido gaussiano adaptativo
        if np.random.rand() > 0.5:
            sigma = noise_sigma * np.std(X_dyn)
            noise = np.random.normal(0, sigma, size=X_dyn.shape)
            X_new = np.concatenate([X_dyn + noise, X_meta], axis=-1)
            X_aug.append(X_new); y_aug.append(y[i])

        # Perturbación temporal (shift ±1)
        if np.random.rand() > 0.7:
            shift = np.random.choice([-1, 1])
            X_shift = np.roll(X_dyn, shift, axis=0)  # roll en eje temporal
            X_new = np.concatenate([X_shift, X_meta], axis=-1)
            X_aug.append(X_new); y_aug.append(y[i])

        # Dropout espacial simulado
        if np.random.rand() > 0.8:
            mask = np.random.binomial(1, 1 - dropout_p, size=X_dyn.shape)
            X_new = np.concatenate([X_dyn * mask, X_meta], axis=-1)
            X_aug.append(X_new); y_aug.append(y[i])

        # Escalado global
        if np.random.rand() > 0.8:
            scale = np.random.uniform(0.95, 1.05)
            X_new = np.concatenate([X_dyn * scale, X_meta], axis=-1)
            X_aug.append(X_new); y_aug.append(y[i])
...
pa_layers = sample_reshaped[:, :, 0, EXCLUDED_TIME_STEPS:]

PA_downsampled = zoom(pa_layers, (0.5, 0.5, 1), order=3)  # nota: ya solo 3 dims
PA_downsampled = PA_downsampled[:, :, :, np.newaxis]  # añadir canal para ConvLSTM

vertices = mod_struct['vertices']
# vertices shape: (N, 3)
# reshape para acoplar con grid
vertices_grid = vertices.reshape(232, 232, 2, 3)
# repetir en el tiempo
time_steps = PA_downsampled.shape[2]
vertices_down = zoom(vertices_grid[:, :, 0, :], (0.5, 0.5, 1), order=1)  # (115,115,3)
vertices_repeated = np.repeat(vertices_down[np.newaxis, :, :, :], time_steps, axis=0)  # (n_times, 115,115,3)
vertices_repeated = vertices_repeated.transpose(1,2,0,3)  # (115,115,n_times,3)

# concatenar como canales adicionales
PA_with_metadata = np.concatenate((PA_downsampled, vertices_repeated), axis=3) 

# Creamos ventanas deslizantes manteniendo estructura espacial
X, y = create_sliding_window_spatial(PA_with_metadata, WINDOW_SIZE)

y = y[:, :, :, :1]  # Solo predecir los canales originales de PA
...
X_train_scaled, y_train_scaled = augment_data(X_train_scaled, y_train_scaled, n_dyn_channels=1)

# Definimos el modelo ConvLSTM2D (ajustado para C=4 en entrada, pero salida C=2)
def create_convlstm_model(n_frames, H, W, C_in, C_out):
    model = Sequential([
        ConvLSTM2D(filters=16, kernel_size=(3, 3), padding='same', activation='relu', return_sequences=True, kernel_initializer=initializer,
                   input_shape=(n_frames, H, W, C_in)),
        ConvLSTM2D(filters=16, kernel_size=(3, 3), padding='same', activation='relu', return_sequences=False, kernel_initializer=initializer),
        Conv2D(filters=C_out, kernel_size=(3, 3), padding='same', activation='linear', kernel_initializer=initializer)
    ])
    model.compile(optimizer=Adam(learning_rate=0.001), loss='mse', metrics=['mae'])
    return model

# Creamos el modelo
n_frames, H, W, C_in = WINDOW_SIZE, PA_with_metadata.shape[0], PA_with_metadata.shape[1], PA_with_metadata.shape[3]
C_out = 1  # Solo predecir los canales originales
model = create_convlstm_model(n_frames, H, W, C_in, C_out)
model.summary()
...
# Predecimos autoregresivamente (ajustado para manejar canales extras en input)
y_pred = np.zeros_like(y_test_scaled)
current_input = X_test_scaled[0:1]  # Incluye canales extras
for t in range(y_test_scaled.shape[0]):
    pred = model.predict(current_input, verbose=0)  # pred es (1, H, W, 2)
    y_pred[t] = pred[0]
    # Actualizar input: shift window, agregar pred a los primeros 2 canales, mantener canales estáticos 3-4
    shifted_input = current_input[:, 1:, :, :, :]       # (1,19,H,W,4)
    last_static = current_input[:, -1:, :, :, 1:]      # (1,1,H,W,3)
    new_step = np.concatenate([pred[:, None, :, :, :], last_static], axis=4)  # (1,1,H,W,4)
    current_input = np.concatenate([shifted_input, new_step], axis=1) # (1, 20, H, W, 4)
...
y_test_denorm = scaler_y.inverse_transform(y_test_scaled.reshape(-1, C_out)).reshape(y_test_scaled.shape)
y_pred_denorm = scaler_y.inverse_transform(y_pred.reshape(-1, C_out)).reshape(y_pred.shape)
...
# Calcular SSIM (corregido para un solo canal)
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
         ...
         mse_per_t, label='MSE ConvLSTM2D')
plt.xlabel('Tiempo t')
...
# Visualización de predicciones vs reales (corregido para un solo canal)
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_pred_denorm[:, pixel_idx_x, pixel_idx_y, 0], label='Predicciones ConvLSTM2D', alpha=0.7)
plt.xlabel('Tiempo t')
plt.ylabel('Valor')
...
# Visualización de predicciones vs reales (zoom primeros 100 t, corregido para un solo canal)
plt.figure(figsize=(12, 6))
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_pred_denorm[:100, pixel_idx_x, pixel_idx_y, 0], label='Predicciones ConvLSTM2D', alpha=0.7)
plt.xlabel('Tiempo t')
plt.ylabel('Valor')
...
# Crear video con imágenes 2D (real, predicha, error) - corregido para un solo canal
...
for t in range(y_test_denorm.shape[0]):
    real_img = y_test_denorm[t, :, :, 0]  # Single channel
    pred_img = y_pred_denorm[t, :, :, 0]  # Single channel
    error_img = np.abs(y_test_denorm[t, :, :, 0] - y_pred_denorm[t, :, :, 0])
    
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(15, 5))
    
    # Imagen real
    im1 = ax1.imshow(real_img, cmap='viridis', vmin=vmin_test, vmax=vmax_test)
    ax1.set_title('Real')
    ax1.axis('off')
    plt.colorbar(im1, ax=ax1, fraction=0.046, pad=0.04)
    
    # Imagen predicha
    im2 = ax2.imshow(pred_img, cmap='viridis', vmin=vmin_pred, vmax=vmax_pred)
    ax2.set_title('Predicha')
    ax2.axis('off')
    plt.colorbar(im2, ax=ax2, fraction=0.046, pad=0.04)
    
    # Error 2D (diferencia absoluta)
    im3 = ax3.imshow(error_img, cmap='hot', vmin=0, vmax=error_vmax)

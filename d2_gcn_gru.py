...
# 6. Visualización
def plot_predictions(predictions, targets, vertices, sample_nodes=[0, 100, 1000], filename="gcn_structured_predictions.png", time_offset=405):
...
def plot_3d_predictions(predictions, vertices, t=0, filename="gcn_structured_3d_predictions.png", time_offset=405):
...
def main():
    ...
    model = GCN(in_channels=20, hidden_channels=64, gru_hidden=32).to(device)
    loss_fn = nn.MSELoss()
    ...
    for epoch in range(50):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(model_dir, "gcn_structured_best.pt"))
    ...
    model.load_state_dict(torch.load(os.path.join(model_dir, "gcn_structured_best.pt")))
    ...
    print(f"GCN Autoregressive: MSE = {test_loss:.6f}, RMSE = {test_rmse:.6f}, MAE = {test_mae:.6f}, Corr = {test_corr:.6f}")
    ...
    plot_predictions(predictions, targets, vertices, filename=os.path.join(model_dir, "gcn_structured_predictions.png"), time_offset=start_time)
    plot_3d_predictions(predictions, vertices, t=0, filename=os.path.join(model_dir, "gcn_structured_3d_predictions.png"), time_offset=start_time)

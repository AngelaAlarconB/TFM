...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_pred_denorm[:100, pixel_idx_x, pixel_idx_y, 0], label='Predicciones Conv3D', alpha=0.7)
...
plt.title('Pérdida durante el Entrenamiento de Conv3D')
plt.grid(True)
plt.legend()
plt.tight_layout()

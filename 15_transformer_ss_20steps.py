...
# Dividir X_train_scaled en train y val (equivalente a validation_split=0.2)
train_idx = int(0.8 * len(X_train_scaled))  # 80% de train para sub-train, 20% para val
X_tr, y_tr = X_train_scaled[:train_idx], y_train_scaled[:train_idx]
X_va, y_va = X_train_scaled[train_idx:], y_train_scaled[train_idx:]

def scheduled_sampling_training(model, X_train, y_train, X_val, y_val, 
                                epochs=50, batch_size=8, tf_initial=1.0, tf_final=0.0,
                                patience=10):
    """
    Entrenamiento con Scheduled Sampling progresivo.
...
print("Entrenando Transformer espaciotemporal con Scheduled Sampling...")
history = scheduled_sampling_training(
    model, X_tr, y_tr, X_va, y_va,
    epochs=EPOCHS, batch_size=BATCH_SIZE,
    tf_initial=1.0, tf_final=0.0, patience=10
)

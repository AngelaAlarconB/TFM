...
model = create_convlstm_model(n_frames, H, W)
model.summary()
...
# Visualización de predicciones vs reales (para un píxel de ejemplo)
pixel_idx_x, pixel_idx_y = 50, 50  # Ejemplo de píxel en (50, 50)
plt.figure(figsize=(12, 6))
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), y_test_denorm[:, pixel_idx_x, pixel_idx_y, 0], 
         label='Valores Reales', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), y_pred_denorm[:, pixel_idx_x, pixel_idx_y, 0], 
         label='Predicciones ConvLSTM2D', alpha=0.7)
plt.xlabel('Tiempo t')

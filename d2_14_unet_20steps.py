...
def create_sliding_window_spatial(data, window_size, start_t=0, end_t=1200):
    ...
    # Reorganizamos para formato: (n_frames, window_size, H, W, C)
...
PA_downsampled = zoom(pa_layers, (0.5, 0.5, 1, 1), order=1) 
...
from tensorflow.keras.layers import UpSampling2D, Concatenate, Lambda, Add
import tensorflow as tf

def create_unet_model(n_frames, H, W, C):
    """
    U-Net con ConvLSTM2D y resize dinámico en skip-connections y residual.
    Entrada: (batch, n_frames, H, W, C)
    Salida: (batch, H, W, C) (align con último frame de entrada)
    """
    inp = Input(shape=(n_frames, H, W, C))
    ...
    e2 = ConvLSTM2D(64, (3, 3), padding='same', return_sequences=True, strides=(2, 2))(e1)
    e2 = BatchNormalization()(e2)
    e2 = Dropout(0.2)(e2)

    e3 = ConvLSTM2D(128, (3, 3), padding='same', return_sequences=True, strides=(2, 2))(e2)
    ...
    # Decoder - nivel 1
    d1 = UpSampling2D((2, 2))(b)            # up
    e2_last = Lambda(lambda x: x[:, -1])(e2)  # último frame de e2

    # resize dinámico e2_last -> tamaño de d1
    e2_resized = Lambda(
        lambda tensors: tf.image.resize(tensors[0],
                                        size=tf.shape(tensors[1])[1:3],
                                        method='bilinear'),
        name='resize_e2_to_d1'
    )([e2_last, d1])

    d1 = Concatenate()([d1, e2_resized])
    d1 = Conv2D(128, (3, 3), padding='same', activation='relu')(d1)
    d1 = BatchNormalization()(d1)
    d1 = Dropout(0.2)(d1)

    # Decoder - nivel 2
    d2 = UpSampling2D((2, 2))(d1)
    e1_last = Lambda(lambda x: x[:, -1])(e1)  # último frame de e1

    # resize dinámico e1_last -> tamaño de d2
    e1_resized = Lambda(
        lambda tensors: tf.image.resize(tensors[0],
                                        size=tf.shape(tensors[1])[1:3],
                                        method='bilinear'),
        name='resize_e1_to_d2'
    )([e1_last, d2])

    d2 = Concatenate()([d2, e1_resized])
    ...
    out = Conv2D(C, (3, 3), padding='same', activation='linear')(out)

    # Residual connection con el último frame de entrada
    last = Lambda(lambda z: z[:, -1])(inp)  # shape = (batch, H_in, W_in, C)

    # Aseguramos que la salida tenga exactamente la misma forma espacial que `last`
    out_resized = Lambda(
        lambda tensors: tf.image.resize(tensors[0],
                                        size=tf.shape(tensors[1])[1:3],
                                        method='bilinear'),
        name='resize_out_to_last'
    )([out, last])

    out = Add()([out_resized, last])  # ahora shapes coinciden
...
model = create_unet_model(n_frames, H, W, C)
model.summary()
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + len(mse_per_t)), mse_per_t, label='MSE U-Net')
plt.xlabel('Tiempo t')
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_pred_denorm[:, pixel_idx_x, pixel_idx_y, 0], label='Predicciones U-Net (Capa 1)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_test_denorm[:, pixel_idx_x, pixel_idx_y, 1], label='Valores Reales (Capa 2)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + y_test_denorm.shape[0]), 
         y_pred_denorm[:, pixel_idx_x, pixel_idx_y, 1], label='Predicciones U-Net (Capa 2)', alpha=0.7)
plt.xlabel('Tiempo t')
plt.ylabel('Valor')
...
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_pred_denorm[:100, pixel_idx_x, pixel_idx_y, 0], label='Predicciones U-Net (Capa 1)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_test_denorm[:100, pixel_idx_x, pixel_idx_y, 1], label='Valores Reales (Capa 2)', alpha=0.7)
plt.plot(range(EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE, 
               EXCLUDED_TIME_STEPS + split_idx + WINDOW_SIZE + 100), 
         y_pred_denorm[:100, pixel_idx_x, pixel_idx_y, 1], label='Predicciones U-Net (Capa 2)', alpha=0.7)
plt.xlabel('Tiempo t')
plt.ylabel('Valor')
...
plt.title('Pérdida durante el Entrenamiento de U-Net')
plt.grid(True)
plt.legend()
plt.tight_layout()
...
with open(os.path.join(model_dir, "metricas.txt"), "w") as f:
    f.write("Resultados Globales para U-Net:\n")
...
# Imprimir métricas
print("\nResultados Globales para U-Net:")
